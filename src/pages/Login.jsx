...
//   - Fixed prefix: "20231000100"  (11 chars)
//   - Variable suffix: 01–99        (2 digits)
//   e.g. 2023100010001 → 2023100010099
const FIXED_PREFIX = "20231000100";

const validateStudentEnrollment = (value) => {
    if (value.length !== 13) return false;
    if (!value.startsWith(FIXED_PREFIX)) return false;
    const suffix = parseInt(value.slice(11), 10);  // last 2 digits
...
const Login = ({ onLogin, onNavigateRegister }) => {
    ...
    const handleLogin = (e) => {
        ...
        // 1️⃣ Check registered users in localStorage first
        ...
        // 2️⃣ Fall back to hardcoded student enrollment pattern (legacy)
        if (validateStudentEnrollment(enrollment)) {
            onLogin("student", { enrollment });
            return;
        }

        // 3️⃣ No match at all
        setError(
            "Enrollment / ID not found. Please register first, or check your credentials."
        );
    };
    ...
    return (
        <div className="login-page">
            ...
            {/* ── LEFT PANEL — campus image + branding ── */}
                    ...
                    {onNavigateRegister && (
                        <p style={{ textAlign: 'center', marginTop: '20px', fontSize: 'clamp(12px,1vw,14px)', color: '#777' }}>
                            ...
                            <button
                                ...
                                style={{ background: 'none', border: 'none', color: '#1a2e5a', fontWeight: 700, cursor: 'pointer', fontSize: 'inherit', textDecoration: 'underline', fontFamily: 'inherit' }}
                            >
                                Sign up
                            </button>
                        </p>
