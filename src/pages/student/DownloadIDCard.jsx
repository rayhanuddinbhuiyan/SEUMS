...
function drawIDCard(canvas, { name, enrollment, department, batch, bloodGroup, contact }) {
    ...
    const px=W-110, py=132, pr=58;
    ctx.beginPath(); ctx.arc(px,py,pr+4,0,Math.PI*2); ctx.strokeStyle="#c8d4ea"; ctx.lineWidth=2; ctx.stroke();
    ctx.beginPath(); ctx.arc(px,py,pr,0,Math.PI*2); ctx.fillStyle="#dde5f0"; ctx.fill();
    ...
    ctx.textAlign="center"; ctx.fillStyle="#8a9ab8"; ctx.font="9px Arial"; ctx.fillText("PHOTO",px,py+pr+16);
    ...
    const rx=LW+28; let fy=78;
    fields.forEach(([label,val],i)=>{
        ctx.textAlign="left"; ctx.fillStyle="#8a9ab8"; ctx.font="9px Arial"; ctx.fillText(label,rx,fy);
        ctx.fillStyle="#1a2e5a"; ctx.font=`bold ${i===0?15:13}px Arial`; ctx.fillText(val,rx,fy+17);
        ctx.strokeStyle="#dde5f0"; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(rx,fy+23); ctx.lineTo(px-75,fy+23); ctx.stroke();
        fy+=37;
    });
    const BH=48;
    ...
    for(let i=0;i<30;i++){const bw=i%4===0?5:2; ctx.fillRect(rx+i*7,H-BH+8,bw,14);}
...
export default function DownloadIDCard() {
    ...
    useEffect(() => {
        if (step === "preview" && canvasRef.current) {
            drawIDCard(canvasRef.current, { ...student, bloodGroup, contact });
        }
    }, [step]);
    ...
    return (
        <div className="idcm-overlay" style={{ position:"relative", minHeight:"100vh", background:"#f0f2f8", alignItems:"flex-start", backdropFilter:"none" }}>
                ...
                {step === "form" ? (
                    <div className="idcm-form-wrap">
                        <div className="idcm-form-header">
                            <span className="idcm-form-icon">🪪</span>
